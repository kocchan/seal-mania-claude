...
const DEFAULT_TAGS = config.wordpress?.defaultTags || ['新作販売', 'ボンボンドロップシール'];
...
class WordPressService {
  ...
  getCategoryIds(frontmatter) {
    const ids = [CATEGORY_MAP['新作情報'], CATEGORY_MAP['抽選・予約情報']];
    const tags = Array.isArray(frontmatter.tags) ? frontmatter.tags : [];

    for (const tag of tags) {
      for (const [keyword, categoryId] of Object.entries(CHARACTER_CATEGORIES)) {
        if (tag.includes(keyword)) ids.push(categoryId);
      }
    }
  ...
  async postArticle(frontmatter, htmlContent, imagePath) {
    const slug = frontmatter.slug || 'new-release-article';
    const mediaId = await this.uploadImage(imagePath, slug);

    const tags = Array.isArray(frontmatter.tags) ? frontmatter.tags : [];
    const excludeTags = new Set(['新作販売', 'ボンボンドロップシール', 'ぷっくりシール', '新作']);
...
async function main() {
  console.log('[WordPress] 新作販売記事投稿開始');
