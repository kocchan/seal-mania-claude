/**
 * 新作情報記事をWordPressに投稿する
 *
 * 使い方:
 *   node scripts/newproduct/post-wordpress.js
 *
 * 入力:
 *   output/newproduct/drafts/{date}/{slug}.md
 *   output/newproduct/images/{date}/{slug}.png (任意・現状未生成)
 *
 * 機能:
...
// 設定読み込み
// =====================================
const configPath = path.join(__dirname, '../../config/newproduct.json');
const config = JSON.parse(fs.readFileSync(configPath, 'utf-8'));

const CONFIG = {
  draftsDir: config.output?.draftsDir || 'output/newproduct/drafts',
  imagesDir: config.output?.imagesDir || 'output/newproduct/images',
  waitMs: 2000,
...
const DEFAULT_TAGS = config.wordpress?.defaultTags || ['新作', 'ボンボンドロップシール'];
...
class WordPressService {
  ...
  async postArticle(frontmatter, htmlContent, imagePath) {
    ...
    const tags = Array.isArray(frontmatter.tags) ? frontmatter.tags : [];
    const excludeTags = new Set(['抽選販売', '新作', 'ボンボンドロップシール', 'ぷっくりシール']);
...
// =====================================
async function main() {
  console.log('[WordPress] 新作情報記事投稿開始');
  ...
  for (const dateDir of dateDirs) {
    ...
    for (const file of files) {
      ...
      // 新作スクリプトは画像必須としない（画像があれば添付、なければアイキャッチなしで投稿）
